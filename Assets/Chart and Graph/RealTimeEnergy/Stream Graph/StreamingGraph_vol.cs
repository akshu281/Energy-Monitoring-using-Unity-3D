...
public class StreamingGraph_vol : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Graph.DataSource.ClearCategory("Device 1"); // clear the "Player 1" category. this category is defined using the GraphChart inspector
    ...
    void Update()
    {
        ...
        if (lastTime + 2f < time)
        {
            ...
            string T = ExecuteAndRead("{ \"emeter\":{ \"get_realtime\":{ } } }", IPAddress.Parse("192.168.137.23"), 10000);
            //string J = ExecuteAndRead("{ \"emeter\":{ \"get_realtime\":{ } } }", IPAddress.Parse("192.168.137.8"), 10000);
            ...
            Graph.DataSource.AddPointToCategoryRealtime("Device 1", System.DateTime.Now, vol); // each time we call AddPointToCategory 
            //Graph.DataSource.AddPointToCategoryRealtime("Player 2", System.DateTime.Now, vol1);
